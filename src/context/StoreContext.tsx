...
interface StoreContextType {
  ...
  // Actions
  recordPayment: (studentId: string, amount: number, note: string, attachment?: string) => Promise<void>;
  recordDistribution: (studentId: string, items: { productId: string; quantity: number }[], note?: string, attachment?: string) => Promise<void>;
  recordStockIn: (productId: string, quantity: number, note?: string) => void;
  updateProductStock: (productId: string, quantity: number, type: 'IN' | 'OUT') => Promise<void>;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const recordDistribution = async (studentId: string, items: { productId: string; quantity: number }[], note: string = '', attachment?: string): Promise<void> => {
    if (!currentUser) return;
    const newTx: Transaction = {
      id: `t${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
      type: 'DISTRIBUTION',
  ...
  const updateProductStock = async (productId: string, quantity: number, type: 'IN' | 'OUT') => {
    if (mode === 'PRODUCTION') {
      try {
        await runTransaction(db, async (transaction) => {
